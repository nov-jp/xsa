...
- **コードを最少化**: `.what-do` のような単一の役割しかない class属性値 を `--what--` と `var(--do)` に置き換えることで、CSSコード を削減できます。
- **!important が不要**: ExStyle の CSSコード は基本的に属性セレクタ1つ分の詳細度しかないため、!important を使うことなく簡単に上書きや無効化ができます。
...
ExStyleプロパティ を機能させるためのパッケージは複数あります。プロジェクトに応じて最適なものが使用できます。
